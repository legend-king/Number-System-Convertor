...
public class BinaryConvertor extends AppCompatActivity {
    Button binaryConvert;
    EditText editBinaryText;
    TextView binaryDecimal, binaryOctal, binaryHexaDecimal;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binaryConvert = findViewById(R.id.binaryConvert);
        editBinaryText = findViewById(R.id.editBinaryConvert);
        binaryDecimal = findViewById(R.id.binary_decimal);
        binaryOctal = findViewById(R.id.binary_octal);
        binaryHexaDecimal = findViewById(R.id.binary_hexadecimal);

        binaryConvert.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String b1 = editBinaryText.getText().toString();
                String dec="", oct="", hexa="";
                if (b1.equals("")) {
                    Toast.makeText(BinaryConvertor.this, "Please Enter a binary value first", Toast.LENGTH_LONG).show();
                }
                else {
                    boolean c = true;
                    for (int i = 0; i < b1.length(); i++) {
                        char a = b1.charAt(i);
                        if (a != '0' && a != '1') {
                            Toast.makeText(BinaryConvertor.this, "Binary number can only contain 0 and 1", Toast.LENGTH_LONG).show();
                            c = false;
                            break;
                        }
                    }
                ...
                binaryDecimal.setText(decAns);
                binaryOctal.setText(octAns);
                binaryHexaDecimal.setText(hexaAns);
